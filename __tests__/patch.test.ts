...
const workdir = 'working-directory'
...
describe('patch', () => {
  describe('construction', () => {
    ...
    it('should construct with default options', () => {
      expect(
        new patch.Patch(patchFile, patch.Patch.defaultOptions)
  ...
  describe('application', () => {
    ...
    it('should apply with added options', async () => {
      ...
      expect(patch.Patch.defaultOptions.strip).not.toEqual(strip)
      expect(
        await new patch.Patch(patchFile).apply(workdir, {strip: strip})
      ).toEqual(0)
      expect(jest.mocked(exec.exec)).toHaveBeenCalledTimes(1)
      expect(jest.mocked(exec.exec)).toHaveBeenCalledWith('patch', [
        `--directory=${workdir}`,
...
describe('manager', () => {
  ...
  describe('applying patches', () => {
    ...
    it('should throw error when failing to apply a patch', async () => {
      ...
      expect(async () => {
        await manager.patch(workdir)
      }).rejects.toThrow(`Failed to apply ${patchFile} in ${workdir}`)
    })
  })
})
